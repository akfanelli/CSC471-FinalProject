...
void createNormals(vector<float> &pos, vector<float> &nor, vector<unsigned int> &ele)
{
	...
	for (int f = 0; f < ele.size(); f += 3)
	{
		int f1 = ele[f];
		int f2 = ele[f + 1];
		int f3 = ele[f + 2];

		glm::vec3 v1 = glm::vec3(pos[3*f1], pos[3*f1 + 1], pos[3*f1 + 2]);
		glm::vec3 v2 = glm::vec3(pos[3*f2], pos[3*f2 + 1], pos[3*f2 + 2]);
		glm::vec3 v3 = glm::vec3(pos[3*f3], pos[3*f3 + 1], pos[3*f3 + 2]);
	 ...
	 for (int a = 0; a < nor.size(); a += 3)
	 {
	 	glm::vec3 v = glm::vec3(nor[a], nor[a + 1], nor[a + 2]);
	 	glm::normalize(v);
	 	nor[a] = v.x;
	 	nor[a + 1] = v.y;
	 	nor[a + 2] = v.z;
...
void Shape::createShape(tinyobj::shape_t & shape)
{
	...
	if (norBuf.empty())
		createNormals(posBuf, norBuf, eleBuf);
}
